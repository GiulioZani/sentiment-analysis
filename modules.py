...
class SelfAttention(nn.Module):
    def __init__(self, embedding_dim, heads):
        ...
        self.heads = heads
        self.to_keys = nn.Linear(embedding_dim,
                                 embedding_dim * heads,
                                 bias=False)
        self.to_queries = nn.Linear(embedding_dim,
                                    embedding_dim * heads,
                                    bias=False)
        self.to_values = nn.Linear(embedding_dim,
                                   embedding_dim * heads,
                                   bias=False)
    ...
    def forward(self, x):
        ...
        keys = self.to_keys(x).view(batch_size, tweet_length, self.heads,
                                    embedding_dim)
        queries = self.to_queries(x).view(batch_size, tweet_length, self.heads,
                                          embedding_dim)
        values = self.to_values(x).view(batch_size, tweet_length, self.heads,
                                        embedding_dim)
        keys = keys.transpose(1,
                              2).contiguous().view(batch_size * self.heads,
                                                   tweet_length, embedding_dim)
        queries = queries.transpose(1, 2).contiguous().view(
            batch_size * self.heads, tweet_length, embedding_dim)
        values = values.transpose(1,
                                  2).contiguous().view(batch_size * self.heads,
                                                       tweet_length,
                                                       embedding_dim)
        ...
        out = torch.bmm(dot, values).view(batch_size, self.heads, tweet_length,
                                          embedding_dim)
        out = out.transpose(1, 2).contiguous().view(batch_size, tweet_length,
...
class Transformer(nn.Module):
    def __init__(self, embedding_dim, seq_length, num_heads, num_tokens, depth,
        ...
        self.token_embedding = nn.Embedding(embedding_dim=embedding_dim,
                                            num_embeddings=num_tokens)
        self.positional_embedding = nn.Embedding(embedding_dim=embedding_dim,
    ...
    def forward(self, x):
        ...
        positions = torch.unsqueeze(
            self.positional_embedding(torch.arange(tweet_length)),
            0).expand(batch_size, tweet_length, embedding_dim)
        x = tokens + positions
        x = self.transformer_blocks(x)
        x = x.max(dim=1)[0]
        x = self.to_probabilities(x)
        return F.log_softmax(x,
                             dim=1)  # todo: why log_softmax instead of softmax
